    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            if (!visited[y * width + x] && maskData.data[(y * width + x) * 4 + 3] > 0) {
                ...
                const pixels = [];
                floodFill(x, y, pixels, maskData, visited, width, height, false /* isHole */);
...
function findHoles(object, maskData, visited, width, height) {
    ...
    for (let y = minY; y <= maxY; y++) {
        for (let x = minX; x <= maxX; x++) {
            if (!visited[y * width + x] && maskData.data[(y * width + x) * 4 + 3] === 0) {
                const hole = [];
                floodFill(x, y, hole, maskData, visited, width, height, true /* isHole */);
                if (hole.length > 0) {
                    const holeContour = traceContour(hole, maskData, width, height, true /* isHole */);
