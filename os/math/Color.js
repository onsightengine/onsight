...
//
//  Additional Source(s)
//      MIT     https://github.com/mrdoob/three.js/blob/master/src/math/Color.js - Nov 16, 2022
//      MIT     https://github.com/at-import/color-schemer/blob/master/stylesheets/color-schemer/_ryb.scss
//      MIT     https://github.com/iskolbin/lryb/blob/master/ryb.lua
//
/////////////////////////////////////////////////////////////////////////////////////
...
class Color {
    ...
    set(r = 0, g, b, format = '') {
        ...
        // No valid arguments passed
            ...
            if (g || b) console.warn(`Color: Passed some valid arguments, however 'r' was ${r}`);
            // nothing to do
    ...
    setHex(hexColor) {
        ...
        if (hexColor > 0xffffff || hexColor < 0) {
            console.warn(`Color: Given decimal outside of range, value was ${hexColor}`);
    ...
    hexString(hexColor /* optional */){
        ...
        return Color.hexString(hexColor);
    }
    ...
    add(color) {
        if (! color.isColor) console.warn(`Color: add() was not called with a 'Color' object`);
    ...
    mix(color, percent = 0.5) {
        if (! color.isColor) console.warn(`Color: mix() was not called with a 'Color' object`);
        percent = clamp(percent, 0, 1);
    ...
    multiply(color) {
        if (! color.isColor) console.warn(`Color: multiply() was not called with a 'Color' object`);
        return this.setRGBF(this.r * color.r, this.g * color.g, this.b * color.b);
    }
    ...
    subtract(color) {
        if (! color.isColor) console.warn(`Color: subtract() was not called with a 'Color' object`);
        return this.setRGBF(this.r - color.r, this.g - color.g, this.b - color.b);
    }
    ...
    equals(color) {
        if (! color.isColor) console.warn(`Color: equals() was not called with a 'Color' object`);
...
function hsl(hexColor, channel = 'h') {
    ...
    switch (channel) {
        ...
        case 's': return _hslS;
        case 'l': return _hslL;
        default: console.warn(`Color: Unknown channel (${channel}) requested in hsl()`);
    }
...
const _mix1 = new Color();
const _mix2 = new Color();
const _random = new Color();
